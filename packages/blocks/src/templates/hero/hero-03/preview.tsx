...
export function Hero03Preview({ block }: BlockTemplatePreviewProps) {
  const title = block.props.title ?? block.type
  const description = block.props.description ?? block.template

  return (
    <TemplateFrame block={block} className="randee-hero-03" initScript={init}>
      <img
        className="randee-hero-03__thumb"
        ...
        height={80}
      />
      <p className="randee-hero-03__label">{block.template}</p>
      <h2 className="randee-hero-03__title">{title}</h2>
      <p className="randee-hero-03__body">{description}</p>
    </TemplateFrame>
  )
}
